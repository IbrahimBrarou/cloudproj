class VmBuildController < ApplicationController
  def create
    vm_params = params.permit(:vm_name, :os_options, :cpus, :memory, :disk_size, packages: [])

    PackerBuildJob.perform_later(vm_params)

    render json: { status: "VM build queued", vm_name: vm_params["vm_name"] }
  end
